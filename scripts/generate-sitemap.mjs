...
// a sitemap pointing at a placeholder host without failing the build. Local runs
// keep a usable default; CI must stop instead.
if (!resolvedSiteUrl && process.env.CI) {
  throw new Error('SITE_URL (or VITE_SITE_URL) must be set in CI: refusing to build a sitemap for a placeholder domain.')
}
